...
class CodeformerConfig(GPT2Config):
    def __init__(self,
                 # chunk_length,
                 # context_length,
                 # max_chunk_num,
...
#                 pad_token_id,
        ...
        self.max_chunk_num = kwargs.get('max_chunk_num')
        self.enc1_n_layers = kwargs.get('enc1_n_layers')
        self.enc2_n_layers = kwargs.get('enc2_n_layers')
        self.dec_n_layers = kwargs.get('dec_n_layers')
...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    config = CodeformerConfig(
        ...
        chunk_length = megatron_args.encoder_seq_length,
        context_length=megatron_args.max_context_length,
        max_chunk_num=megatron_args.max_sent_num,
        n_embd=megatron_args.hidden_size,
        enc1_n_layers = megatron_args.encoder_1_num_layers,
        enc2_n_layers = megatron_args.encoder_2_num_layers,
        dec_n_layers = megatron_args.decoder_num_layers,
        n_head=megatron_args.num_attention_heads,
        n_inner=megatron_args.ffn_hidden_size,
        activation_function=activation_function,
#        resid_pdrop=0.1,
#        embd_pdrop=0.1,
        hid_pdrop=megatron_args.hidden_dropout,
        attn_pdrop=megatron_args.attention_dropout,
...
#        summary_first_dropout=0.1,
        ...
        use_cache=True,
        # TODO add bos and eos ids to args
        bos_token_id=1,
        eos_token_id=2,
        pad_token_id = megatron_args.pad_id,
        architectures=["CodeformerLangModeling"],
    )
    ...
    #     tp_state_dicts[0], "model.codeformer_model.embedding.position_embeddings.weight"
    # )
    # output_state_dict["transformer.wpe.weight"] = position_embeddings.to(dtype)

    # Convert and store the word embeddings.
    # TODO take 'codeformer_model' key from somewhere
    word_embeddings = torch.cat(
        [
            get_element_from_dict_by_path(
                tp_state_dicts[tp_rank], "model.codeformer_model.embedding.word_embeddings.weight"
            )
    ...
    output_state_dict["codeformer.wte.weight"] = word_embeddings

    # Transformer Layers
    print("Converting codeformer layers")
    # The number of heads.
    heads = config.n_head
    ...
    num_layers = dict()
    num_layers['encoder_1'] = config.enc1_n_layers // pp_size
    num_layers["encoder_2"] = config.enc2_n_layers // pp_size
    num_layers["decoder"] = config.dec_n_layers // pp_size
    ...
    for pp_rank in range(pp_size):
        ...
        # # The transformer.
        # path = (
        #     "model.codeformer_model.transformer"
        #     if "transformer" in get_element_from_dict_by_path(tp_state_dicts[0], "model.codeformer_model").keys()
        #     else "model.codeformer_model.encoder"
        # )

        not_blocks = ['embedding', 'embedding_dec', 'linear']
        blocks = list(get_element_from_dict_by_path(tp_state_dicts[0], "model.codeformer_model").keys())
        blocks = [x for x in blocks if x not in not_blocks]
        paths = [f"model.codeformer_model.{block}" for block in blocks]
        ...
        for path, block in zip(paths, blocks):
            ...
            for key, val in get_element_from_dict_by_path(tp_state_dicts[0], path).items():
                ...
                layer_idx = int(m.group(1)) + pp_rank * num_layers[block]
                ...
                layer_name = f"codeformer.{block}.h.{layer_idx}"
            ...
            if block!='linear' and num_layers[block] != (layer_idx + 1):
                raise ValueError(f"Expected {num_layers[block]} layers but found {layer_idx + 1}")

        params = get_element_from_dict_by_path(tp_state_dicts[0], "model.codeformer_model.linear")
        output_state_dict["codeformer.linear.weight"] = params["weight"].to(dtype)
        output_state_dict["codeformer.linear.bias"] = params["bias"].to(dtype)

    # The final layernorm.
    print("Converting final layernorm")
    params = get_element_from_dict_by_path(tp_state_dicts[0], str(path))
    output_state_dict["codeformer.ln_f.weight"] = params["final_layernorm.weight"].to(dtype)
    output_state_dict["codeformer.ln_f.bias"] = params["final_layernorm.bias"].to(dtype)
...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    """
    # TODO may be we need to write it
    print("Not implemented for codeformer!!!")
    return None
    ...
    for i in range(args.target_tensor_model_parallel_size):
        pos_emb_dict = get_element_from_dict_by_path(
            output_state_dict[i], "model.codeformer_model.embedding.position_embeddings"
        )
        pos_emb_dict["weight"] = pos_embedding

        word_emb_dict = get_element_from_dict_by_path(
            output_state_dict[i], "model.codeformer_model.embedding.word_embeddings"
        )
    ...
    for pp_rank in range(args.target_pipeline_model_parallel_size):
        ...
        for layer in range(num_layers):
            ...
            for layer_name in layers_to_copy:
                ...
                for i in range(args.target_tensor_model_parallel_size):
                    params_dict = get_element_from_dict_by_path(output_state_dict[i], "model.codeformer_model.encoder")
